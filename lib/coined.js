...
var utils = require('./utils')
  , network = bcoin.protocol.network
  , seeds = network.seeds;
...
function Coined(options) {
  ...
  if (options.noPreload) {
    network.preload = network._preload;
  }
  ...
  this.db.get('meta/version', function(err, data) {
    ...
    return self.db.put('meta/version', { version: self.version }, function(err) {
...
Coined.prototype._clearPreload = function() {
  utils.merge(network._preload = {}, network.preload);
  utils.merge(network.preload, {
    'v': 1,
    'type': 'chain',
    'hashes': network._preload.hashes.slice(0, 1),
    'ts': network._preload.ts.slice(0, 1),
    'heights': network._preload.heights.slice(0, 1)
  });
  if (this.pool) {
...
Coined.prototype._encrypt = function(data, passphrase) {
  if (data[0] === ':') {
    return this._error('Already encrypted.');
  }
...
Coined.prototype.sendTo = function(address, amount, callback, from) {
  ...
  function done(err, status, hash) {
    ...
    var isOwn = this.accounts.some(function(account) {
...
Coined.prototype._sendTo = function(address, amount, callback, from) {
  ...
  if (amount.cmpn(0) !== 0) {
    tx.output({ address: address, value: amount });
  }
...
Coined.prototype._fill = function(from, tx, callback) {
  ...
  var unspent = utils.uniq(this.accounts.reduce(function(out, account) {
    var unspent = account.unspent();
    return out.concat(unspent);
  }));
  ...
  var result = tx.fillUnspent(unspent, change);
  if (!result) {
  ...
  tx.inputs.forEach(function(input, i) {
    this.accounts.forEach(function(account) {
      if (!input.out.tx || !account.ownOutput(input.out.tx))
        return;
      tx.scriptInput(input, account.getPublicKey());
      tx.signInput(input, account.key, 'all');
    }, this);
  }, this);
...
Coined.prototype.ownInput = function(tx, index) {
  ...
  if (index != null && !tx.inputs[index])
    return false;

  own = this.accounts.map(function(account) {
    return account.ownInput(tx, index);
  });

  own = own.reduce(function(out, inputs) {
    if (!inputs) return out;
    inputs.forEach(function(input) {
      if (!~out.indexOf(input)) {
        out.push(input);
      }
    });
    return out;
...
Coined.prototype.ownOutput = function(tx, index) {
  ...
  if (index && typeof index === 'object')
    index = tx.outputs.indexOf(index);

  if (index != null && !tx.outputs[index])
    return false;
  ...
  own = own.reduce(function(out, outputs) {
    if (!outputs) return out;
    outputs.forEach(function(output) {
      if (!~out.indexOf(output)) {
        out.push(output);
      }
    });
    return out;
...
Coined.prototype.addTX = function(tx, noWrite) {
  var self = this;
  this.accounts.forEach(function(account) {
    if (!tx.ts || !tx.block) {
      self.pending[tx.hash('hex')] = tx;
    } else {
      account.tx.add(tx, noWrite);
    }
    self.pool.watch(tx.hash('hex'));
  });
};
...
Coined.prototype.sign = function(msg, account) {
  ...
  var sig = bcoin.ecdsa.sign(msg, account.key).toDER('hex');
...
Coined.prototype.getInputData = function(input) {
  return bcoin.tx.getInputData(input);
};

Coined.prototype.getOutputData = function(output) {
  return bcoin.tx.getOutputData(output);
};

Coined.inspectScript = function(input, output) {
  return bcoin.script.format(input, output);
};

Coined.prototype.getScriptSig = function(input) {
  return Coined.inspectScript(input, null).join(' ');
};

Coined.prototype.getScriptPubkey = function(output) {
  return Coined.inspectScript(null, output).join(' ');
};
...
Coined.prototype.getUnconfirmedTransactions = function(callback) {
  var self = this
    , txs = [];
...
Coined.prototype.parseTX = function(tx, block) {
  ...
  tx.inputs.forEach(function(input) {
    ptx.inputs.push({
      out: {
        tx: null,
        // tx: input.out.tx ? self.parseTX(input.out.tx) : null,
        hash: input.out.hash,
  ...
  ptx.inputs.forEach(function(input, i) {
    var keys = self.getInputData(input);
    if (!keys) return;
  ...
  ptx.inputs.forEach(function(input) {
    ...
    // outputs array.
    if (ptx.inputs.length === 1 && +input.out.hash === 0) {
      ptx.coinbase = true;
    }
  ...
  // Derive scriptSig notation from output Script
  ptx.inputs.scriptSigs = [];
  ptx.inputs.forEach(function(input) {
    var code = self.getScriptSig(input);
    if (!code) return;
    ptx.inputs.scriptSigs.push(code);
    input.scriptSigs = code;
  });

  // Derive output addresses from output Script
  ptx.outputs.addresses = [];
  ptx.outputs.forEach(function(output) {
    var keys = self.getOutputData(output);
    if (!keys) return;
