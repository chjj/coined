...
function Coined(options) {
  ...
  this.db.get('meta/version', function(err, ver) {
    if (ver) {
      self.previousVersion = ver;
    }
    return self.db.set('meta/version', self.version, function(err) {
      if (err) self.emit('error', err);
      self.log('Version written: %s, previous: %s',
...
Coined.prototype.ubalance = function balance() {
  ...
  Object.keys(this.pending).forEach(function(hash) {
    ...
    self.accounts.forEach(function(account) {
      ...
      ownInput.forEach(function(input) {
        if (input.out.tx) {
          var value = input.out.tx.outputs[input.out.index].value;
          total.isub(input.value);
        }
      });
      ownOutput.forEach(function(output) {
...
Coined.prototype.parseTX = function(tx, block) {
  ...
  ptx.inputs.forEach(function(input, i) {
    ...
    input.out.value = tx.inputs[i].out.tx
      ? tx.inputs[i].out.tx.outputs[input.out.index].value.clone()
      : null;
  });
