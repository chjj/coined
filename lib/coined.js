...
Coined.prototype.ubalance = function balance() {
  ...
  this.accounts.forEach(function(account) {
    return account.unspent().forEach(function(item) {
      var value = item.tx.outputs[item.index].value;
      if (item.tx.block) {
        return confirmed.iadd(value);
      }
      return unconfirmed.iadd(value);
    });
  });
...
Coined.prototype._fill = function(from, tx, callback) {
  ...
  tx.out(this.account, new bn(0));
  // tx.out(this.createAccount(), new bn(0));
...
bcoin.wallet.prototype.sign = function sign(tx, type, inputs, off) {
  if (!type)
    type = 'all';
...
Coined.prototype.parseBlock = function(_block) {
  var self = this;
  var block = _block;
  ...
  block = utils.deepMerge({}, block);

  block._hash = hash;
  block.rhash = utils.revHex(hash);
  ...
  if (block.height === -1) {
    var prevProbe = this.pool.chain._probeIndex(block.prevBlock, block.ts);
    if (prevProbe) {
      // this.pool.chain._addIndex(block._hash, block.ts, prevProbe.height + 1);
  ...
  if (block.ts) {
    block.date = new Date(block.ts * 1000).toISOString()
  }

  utils.hideProperty(block, '_raw');

  utils.hideProperty(_block, 'd', block);
  utils.hideProperty(block, 'o', _block);

  return block;
};

Coined.prototype.parseTX = function(_tx, block) {
  var self = this;
  var tx = _tx;

  if (!tx || typeof tx !== 'object' || !tx.verify || tx.rhash) {
    if (tx && tx.hash && block && block.confirmations && !tx.confirmation) {
  ...
  tx = utils.deepMerge({}, tx);

  tx._hash = hash;
  tx.rhash = utils.revHex(hash);
  tx.rblock = tx.block ? utils.revHex(tx.block) : null;
  tx.verified = _tx.verify();
  tx.ts = _tx.ts;
  ...
  tx.inputs.forEach(function(input) {
    ...
    if (!keys) return;
    tx.inputs.addresses.push(keys.addr);
  ...
  tx.inputs.forEach(function(input) {
    ...
    // outputs array.
    if (tx.inputs.length === 0 && +input.out.hash === 0) {
      tx.coinbase = true;
    }

    if (tx.lock > 0 && input.seq !== 0xffffffff) {
      tx.finalBlockHeight = input.seq;
      tx.finalTimestamp = tx.lock;
    }
  });

  // Derive output addresses from output Script
  tx.outputs.addresses = [];
  tx.outputs.forEach(function(output) {
    var keys = self.getOutputKeys(output);
    if (!keys) return;
    tx.outputs.addresses.push(keys.addr);
    output.address = keys.addr;
  });

  // Derive scriptPubKey notation from output Script
  tx.outputs.scriptPubKeys = [];
  tx.outputs.forEach(function(output) {
    var code = self.getScriptPubKey(output);
    if (!code) return;
    tx.outputs.scriptPubKeys.push(code);
    output.scriptPubKey = code;
  });

  // Convert output values to BTC
  tx.outputs.bvalues = [];
  tx.outputs.total = new bn(0);
  tx.outputs.forEach(function(output) {
    tx.outputs.total = tx.outputs.total.add(output.value);
    var bvalue = utils.toBTC(output.value);
    tx.outputs.bvalues.push(bvalue);
    output.bvalue = bvalue;
  });
  tx.outputs.btotal = utils.toBTC(tx.outputs.total);

  // Grab confirmations from block
  if (!tx.block || !tx.verified) {
    tx.confirmations = 0;
    tx.unconfirmed = true;
  ...
  tx.balance = tx.outputs.total.clone();
  tx.bbalance = utils.toBTC(tx.balance);
  tx.fee = new bn(0);
  tx.bfee = utils.toBTC(tx.fee);
  tx.feeUncertain = true;
  ...
  if (tx.outputs.length <= 2) {
    tx.estimated = tx.outputs[0].value.clone();
    tx.bestimated = utils.toBTC(tx.estimated);
    tx.sender = tx.inputs.addresses[0];
    tx.recipient = tx.outputs.addresses[0];
  } else {
    tx.estimated = tx.balance.clone();
    tx.bestimated = utils.toBTC(tx.balance);
    tx.estimateUncertain = true;
    tx.sender = null;
    tx.recipient = null;
  }
  ...
  // Timestamp
  if (tx.ts) {
    tx.date = new Date(tx.ts * 1000).toISOString()
  }
  ...
  utils.hideProperty(tx, '_raw');

  utils.hideProperty(_tx, 'd', tx);
  utils.hideProperty(tx, 'o', _tx);
